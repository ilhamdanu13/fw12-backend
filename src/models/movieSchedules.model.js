...
exports.selectMovieSchedules = (id, cb) => {
  const sql = `select array_agg(mst.time) as times, m.title, ms.price, c.id as cinemaId, c.name as cinema, c.address, c.city, c.picture as cinemaPicture
  ...
  group by m.title, ms.price, c.name, c.address, c.city, c.picture, c.id`;
  const value = [id];
